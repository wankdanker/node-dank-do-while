 ...
 */
export function doWhile (fn: (more: (cont: boolean) => void) => void, done?: () => void, concurrent: number = 1) {
  let pending = 0
  let end = false
  ...
  function run (fn: (more: (cont: boolean) => void) => void) {
    setTimeout(function () {
      pending += 1
      fn(function (cont: boolean) {
        pending -= 1
